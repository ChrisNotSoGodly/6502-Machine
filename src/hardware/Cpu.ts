...
import { MMU } from "./MMU";
import { instructionSet } from "./imp/Instruction-Set";
...
export class Cpu extends Hardware implements ClockListener {
    ...
    private mmu: MMU;
    private _PC: number = 0x0000; // Program Counter
    private _ACC: number = 0x00; // Accumulator
    private _IR: number = 0x00; // Instruction Register
    private _zFLAG: number = 0x00; // zFlag Register
    private _xReg: number = 0x00; // X Register
    private _yReg: number = 0x00; // Y Register
    private _Step: number = 0x00; // Step Register
    private _operand: number[] = []; // Operand bytes
    private instructionSet: Map<number, import("./imp/Instruction-Set").InstructionMetadata>; // Instruction Set & Metadata
    private stopSystemCallback?: () => boolean;
    public interruptController: InterruptController;
    private keyboardOutputBuffer: string[];
    private memoryWriteAddress: number = 0x0100; // Address to write keyboard input
    private cycleStage: "fetch" | "decode" | "execute" | "interrupt" = "fetch";
    private currentInstructionMetadata?: import("./imp/Instruction-Set").InstructionMetadata;
    private decodeOperandBytesRemaining: number = 0;
    ...
    constructor(mmu: MMU, keyboardOutputBuffer: string[], stopSystemCallback?: () => boolean) {
        super(0, "Cpu");
        this.mmu = mmu;
        ...
        switch (this._IR) {
            ...
            case 0xFF: {
                ...
                if (this._xReg === 0x01) {
                    ...
                    const character = ASCII.byteToChar(this._yReg);
    ...
    public interuptCheck(): void {
        ...
        const asciiValue = ASCII.charToByte(character);
    ...
    public pulse(): void { //this is the new pipeline so everything happens per clock cycle.
        if (!this.running) {
            return;
        }//if
